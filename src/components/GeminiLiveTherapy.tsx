...
export const GeminiLiveTherapy = ({ onBack }: GeminiLiveTherapyProps) => {
    const { isConnected, isSpeaking, isListening, error, startSession, stopSession } = useGeminiLive();
    ...
    return (
        <div className="max-w-6xl mx-auto">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="p-4 bg-gray-900/50 rounded-lg">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className={`text-sm font-medium ${isConnected ? 'text-green-400' : 'text-gray-500'
                                        }`}>
                ...
                <Card className="bg-gray-800/50 border-gray-700">
                    <CardContent className="p-6 flex flex-col items-center justify-center h-full min-h-[400px]">
                        <div className="relative">
                            {/* AI Avatar */}
                            <div className={`w-32 h-32 rounded-full bg-gradient-primary flex items-center justify-center transition-all duration-300 ${isSpeaking ? 'scale-110 shadow-glow' : isListening ? 'scale-105' : 'scale-100'
                                }`}>
                            ...
                            {(isListening || isSpeaking) && (
                                <div className="absolute -inset-4">
                                    <div className={`absolute inset-0 rounded-full ${isSpeaking ? 'bg-purple-500/30' : 'bg-blue-500/30'
                                        } animate-ping`} />
                                </div>
                            )}
                        </div>
            ...
            <div className="mt-6 p-3 bg-gray-900/50 rounded-lg text-xs text-gray-500">
                <p>Model: gemini-2.0-flash-exp | Mode: Live Audio | API: Google Gemini Multimodal</p>
            </div>
        </div>
    );
};
