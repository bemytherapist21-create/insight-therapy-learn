...
export const ServiceCard = ({ icon: Icon, title, description, link, gradient, index }: ServiceCardProps) => {
    return (
        <Link to={link}>
            <Card className="glass-card hover-lift cursor-pointer group animate-scale-in"
                style={{ animationDelay: `${index * 0.1}s` }}>
                <CardHeader className="text-center">
                    <div className={`w-16 h-16 mx-auto rounded-2xl bg-gradient-to-r ${gradient} p-4 mb-4 group-hover:shadow-glow transition-all duration-300`}>
