...
class ResearchService {
    ...
    async generateInsight(query: string): Promise<ResearchResult | null> {
        if (!this.apiKey) {
            logger.error('Perplexity API key is missing. Please set VITE_PERPLEXITY_API_KEY.');
            return null;
        }
        ...
        try {
            ...
            logger.error('Failed to generate insight', error instanceof Error ? error : undefined);
            return null;
        }
    }
}
