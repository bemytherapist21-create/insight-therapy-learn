...
export function useGeminiVoiceTherapy() {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      recognitionRef.current.onerror = (event: any) => {
        ...
        if (
          ...
          toast.error(
            "Microphone access denied. Please click the 🎤 icon in your browser address bar and allow microphone access.",
          );
        } else if (event.error === "network") {
          toast.error(
            "Network error. Please check your internet connection and microphone permissions.",
          );
        } else {
  ...
  const startListening = async () => {
    ...
    if (navigator.permissions && navigator.permissions.query) {
      try {
        ...
        if (permissionStatus.state === "denied") {
          toast.error(
            "Microphone access blocked. Please click the 🔒 icon in your browser address bar and allow microphone access, then reload the page.",
          );
          return;
        }
      } catch (err) {
    ...
    try {
      ...
      toast.error(
        "Failed to start microphone. Please check your browser permissions.",
      );
    }
  };
  ...
  const processUserMessage = async (transcript: string) => {
    try {
      ...
      const safety: VoiceSafetyData = {
        ...
        riskLevel: (safetyData.riskLevel || data.riskLevel || "clear") as
          | "clear"
          | "clouded"
          | "critical",
        colorCode: safetyData.colorCode || data.colorCode || "#10b981",
        requiresIntervention:
          safetyData.requiresIntervention || data.requiresIntervention || false,
        crisisDetected:
          safetyData.crisisDetected || data.crisisDetected || false,
      };
      ...
      if (error instanceof Error) {
        if (
          ...
          errorMsg =
            "Please log in to use voice therapy. Redirecting to login...";
          ...
          errorMsg =
            "Therapy service unavailable. Please try again in a moment.";
        } else {
