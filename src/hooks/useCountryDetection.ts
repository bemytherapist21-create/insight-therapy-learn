...
const CRISIS_RESOURCES: Record<string, CrisisResource[]> = {
  ...
  'DE': [
    ...
    { name: "Kinder & Jugendtelefon", number: "0800 111 0 333", type: 'hotline' }
  ],
  'FR': [
...
export const useCountryDetection = () => {
  ...
  const [resources, setResources] = useState<CrisisResource[]>(CRISIS_RESOURCES['US']);
      ...
      try {
        ...
        if (cached) {
          ...
          if (cacheAge < 24 * 60 * 60 * 1000) {
            ...
            setResources(CRISIS_RESOURCES[parsedCache.country.code] || DEFAULT_RESOURCES);
            setLoading(false);
            return;
          }
        }
