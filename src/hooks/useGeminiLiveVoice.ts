  ...
  const startSession = useCallback(async () => {
    ...
    try {
      if (!navigator.mediaDevices?.getUserMedia) {
        throw new Error(
          "Microphone is not supported in this browser. Please use Chrome, Edge, or Safari.",
        );
      }

      // getUserMedia requires a secure context (HTTPS) except for localhost.
      // This commonly differs between "phone (https production)" and "laptop (http LAN / file)".
      if (
        ...
        const securityErr = new Error(
          "Microphone access requires HTTPS (or http://localhost). Please open the app on a secure URL and try again.",
      ...
      if (!AudioContextCtor) {
        throw new Error(
          "Web Audio is not supported in this browser. Please use Chrome, Edge, or Safari.",
        );
      }

      // Some browsers/devices may reject custom sampleRate; fall back safely.
      let inputAudioContext: AudioContext;
      let outputAudioContext: AudioContext;
      try {
        inputAudioContext = new AudioContextCtor({ sampleRate: 16000 });
      } catch {
        inputAudioContext = new AudioContextCtor();
      }
      try {
        outputAudioContext = new AudioContextCtor({ sampleRate: 24000 });
      } catch {
      ...
      if (err?.name === "SecurityError") {
        setError(
          err.message || "Microphone access requires HTTPS (or localhost).",
        );
        ...
        setError(
          err.message || "Failed to start voice session. Please try again.",
        );
      }
